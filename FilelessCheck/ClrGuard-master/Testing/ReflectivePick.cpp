...
void ReflectionLoad(wchar_t * filePath, int version)
{
    ...
    bstr_t bstrAssemblyName("InjectExample");
    _AssemblyPtr spAssembly = NULL;

    // The .NET class to instantiate.
    bstr_t bstrClassName("InjectExample.Program");
    ...
    fileBuf = malloc(size);
    ReadFile(hFile, fileBuf, size, &high, 0);
    ...
    SafeArrayLock(arr);
    memcpy(arr->pvData, fileBuf, size);
    SafeArrayUnlock(arr);
    ...
    InvokeMethod(spType, L"AnotherFunction", L"HelloThar");
